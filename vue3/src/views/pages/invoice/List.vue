<template>
  <v-dialog v-model="dialogPrint">
    <v-card>
      <v-card-text>
        <div v-if="typeof id === 'number'" id="printContent" ref="printContent" >
          <Invoice :id="id" />
        </div>
      </v-card-text>
      <v-card-actions class="justify-end">
        <v-btn @click="invoiceLoaded()" color="primary">Print</v-btn>
        <v-btn @click="invoiceClose()">Close</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>

  <v-card-title class="d-flex align-center pa-0 pb-2">
    <router-link to="/invoice/add" class="add-invoice" color="white" >
      <v-btn
        class="text-none text-subtitle-1"
        color="primary"
        variant="flat"
      >
        <v-icon>mdi-plus</v-icon> New invoice
      </v-btn>
    </router-link>
    <v-spacer></v-spacer>
...
export default {
  ...
  data: () => ({
    ...
    selected: [],
    headers: [
      { title: 'Name', align: 'start', sortable: false, key: 'customer_name' },
  ...
  methods: {
    ...
    deleteItemConfirm() {
      axiosInstance.delete(Config.API_ENDPOINT + '/invoice/' + this.editedItem.id)
      .then(response => {
        this.initialize();
      })
      .catch(error => {
    ...
    save() {
      ...
      axiosInstance.post(Config.API_ENDPOINT + '/invoiceDeleteAll/' , data)
        .then(response => {
          this.initialize();
        })
        .catch(error => {
